...
// This test verifies CPU NTT with multiple DISTINCT moduli (RNS-style) using
...
int main(int argc, char* argv[])
{
    ...
    cout << "\n=== Testing CPU RNS Forward NTT (X^n + 1) with DISTINCT 61-bit "
            "primes ==="
         << endl;
    ...
    vector<vector<TestDataType>> ntt_mult_result(total_batch);
    ...
    // Comparing CPU NTT multiplication results with schoolbook multiplication
    bool check = true;
    for (int i = 0; i < total_batch; i++)
    {
        int mod_idx = i % MOD_COUNT;
        ...
        if (!check)
        {
            cout << "CPU RNS NTT (X^n + 1) FAILED (in poly " << i
        ...
        {
            cout << "All Correct - CPU RNS NTT (X^n + 1) with "
